module Abilities
  class Administrator
    ...
    def initialize(user)
      ...
      can [:read, :create], Poll::Question
      can [:update, :destroy], Poll::Question do |question|
        question.poll.allow_update?
      end
      can [:read, :create, :documents, :order_answers], Poll::Question::Answer
      can [:update, :destroy], Poll::Question::Answer do |answer|
        answer.question.poll.allow_update?
      end

      can [:create, :update, :destroy], Poll::Question::Answer::Video do |video|
        video.answer.question.poll.allow_update?
      end

      can [:create, :destroy], Image do |image|
        image.imageable_type == "Poll::Question::Answer" && image.imageable.question.poll.allow_update?
      end

      can [:create, :destroy], Document do |document|
        document.documentable_type == "Poll::Question::Answer" && document.documentable.question.poll.allow_update?
      end
