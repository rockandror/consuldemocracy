class Poll < ApplicationRecord
  ...
  validate :end_date_is_not_past_date, unless: :new_record?
  ...
  scope :recounting, -> { where(ends_at: (Time.current - RECOUNT_DURATION)..Time.current) }
  ...
  def end_date_is_not_past_date
    if ends_at_changed? && Date.current.beginning_of_day > ends_at
      errors.add(:ends_at, I18n.t("errors.messages.date_is_not_past_date"))
    end
  end
