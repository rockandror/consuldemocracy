...
class Census::Local::Importation
  ...
  private
    ...
    def process_row(row)
      census_local = build_census_local(row)

      if census_local.invalid?
        invalid_records << census_local
      else
        census_local.save
        created_records << census_local
      end
    end

    def build_census_local(row)
      census_local = Census::Local.new
      census_local.attributes = row.to_hash.slice(*ATTRIBUTES)
      census_local
    end
