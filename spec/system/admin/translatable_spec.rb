...
describe "Admin edit translatable records", :admin do
  ...
  context "Update a translation" do
    ...
    context "CKEditor fields" do
      let(:translatable) { create(:poll_question_answer, poll: create(:poll, starts_at: Date.current)) }
    ...
    context "Change value of a translated field to blank" do
      let(:translatable) { create(:poll, starts_at: Date.current) }
  ...
  context "Remove a translation with invalid data" do
    let(:translatable) { create(:poll_question, poll: create(:poll, starts_at: Date.current)) }
