...
describe "Images", :admin do
  ...
  context "Add image to answer" do
    ...
    scenario "Is not possible for an already started poll" do
      ...
      expect(page).to have_content "It is not possible to create images for an already started poll."

      expect(answer.images).to be_empty
      expect(page).to have_current_path(admin_answer_images_path(answer))
    end
  end
  ...
  context "Remove image from answer" do
    ...
    scenario "Is not possible for an already started poll" do
      ...
      expect(page).to have_content "It is not possible to delete images for an already started poll."
      expect(page).to have_current_path(admin_answer_images_path(answer))
