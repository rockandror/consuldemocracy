...
describe CensusCaller do
  ...
  describe "#call" do
    ...
    it "returns data from local_census_records when remote apis are disabled and do not
        not call external census apis" do
      ...
      expect_any_instance_of(CensusApi).not_to receive(:call)
      expect_any_instance_of(RemoteCensusApi).not_to receive(:call)
    ...
    describe "CensusApi" do
      before { Setting["feature.legacy_remote_census"] = true }

      it "returns census api response when it's available, feature is enabled and census api
          response is valid" do
