...
describe "Wizard Verification Handlers" do
  ...
  context "Index" do
    scenario "Display default custom verification process settings" do
      ...
      expect(page).to have_content "Configure the local census to be used in the verification " \
                                   "of a user in the application."

      expect(page).to have_content "Verify a user's phone"
      expect(page).to have_content "Configure user's phone verification to use it during the " \
