...
describe "Auditable" do
  describe ".auditing_enabled" do
    ...
    it "responds true for Tag and Tagging classes which does not inherit from ApplicationRecord" do
      expect(Tag.auditing_enabled).to be(true)
      expect(Tagging.auditing_enabled).to be(true)
    end
  ...
  describe ".audit_associated_with" do
    ...
    it "returns the associated_with_by_class by the current class name if defined" do
      ...
      expect(Proposal.audit_associated_with).to be_blank
    end
  end
  ...
  describe "on auditable update" do
    it "keeps track of resources and theirs translations from auditing enabled classes" do
      ...
      expect { investment.update!(title: "Updated title", price: 2000) }.to change { Audit.count }.by(2)
    end
    ...
    it "translation related audits are associated with the globalized model instance" do
      ...
      expect(investment.own_and_associated_audits).to match_array([Audit.last])
    end
