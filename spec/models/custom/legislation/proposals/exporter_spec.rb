  ...
  describe "#to_csv" do
    scenario "generate csv with answer content" do
      I18n.with_locale(:es) do
        ...
        csv_contents = "ID,Título de la propuesta,Descripción de la propuesta,Votos totales," \
                       "Número de comentarios,ID del autor,Fecha de nacimiento del autor," \
                       "Localidad del autor,Género del autor\n" \
                       "#{proposal.id},#{proposal.title},#{proposal.description}," \
                       "#{proposal.cached_votes_total},#{proposal.comments_count},#{proposal.author.id}," \
                       "#{proposal.author.date_of_birth},#{proposal.author.geozone.name},#{user_gender}\n"
