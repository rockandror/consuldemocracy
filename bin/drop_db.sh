...
function database_selection() {
  ...
  echo "If you leave this in blank, the default db will be deleted, you can see it in config/database.yml"
  echo ""
...
if [[ $ACCEPT != "true" ]]; then
  confirmation "Are you sure that you want to delete the database?"
...
if [[ $ACCEPT != "true" ]]; then
  echo ""
  confirmation "Are you really really sure that you want to delete the database?"
...
echo ""
if [[ -z $DB_NAME ]]; then
  echo "Database ${DB_NAME}_$ENV will be deleted..."
  echo ""

  RAILS_ENV=$ENV DATABASE=$DB_NAME bundle exec rake db:drop
else
  RAILS_ENV=$ENV bundle exec rake db:drop
fi
